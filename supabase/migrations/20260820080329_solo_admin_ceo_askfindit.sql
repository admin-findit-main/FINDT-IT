-- The only operator admin is ceo@askfindit.com (Google Workspace).
...
as $$
  select exists (
    ...
    where p.id = auth.uid()
      ...
      and lower(p.email) = 'ceo@askfindit.com'
  );
$$;
...
as $$
begin
  if lower(coalesce(new.email, '')) = 'ceo@askfindit.com' then
...
where account_type = 'admin'
  and lower(email) is distinct from 'ceo@askfindit.com';

update public.profiles
set account_type = 'admin'
where lower(email) = 'ceo@askfindit.com'
